...
const watchlist = document.getElementById("movie-lists");
const myWatchlists = (JSON.parse(localStorage.getItem("movieWatchLists"))) || [];
watchlist.textContent = `My Watchlists (${myWatchlists.length})`; 

const moviesSection = document.getElementById("movies-list");

async function getWatchlist() {
 try {
    for(const movieForm of myWatchlists) {
        const urlIdMovie =`https://api.themoviedb.org/3/movie/${movieForm}`;

        const response = await fetch(urlIdMovie, fetchData);
        const watchlistData = await response.json();
         let genreMovies = "";
        for(const genres of watchlistData.genres) {
        ...
        <div class="bg-dark-card bg-transparent grid grid-cols-1 md:grid-cols-[190px_1fr] gap-5 px-4 md:px-10 py-4 text-center md:py-6 ">
            <img src="https://image.tmdb.org/t/p/w500${watchlistData.poster_path}" alt="Poster" class="w-[200px] md:w-full h-auto object-cover rounded-md mx-auto md:mx-0" >
            <div class="flex flex-col gap-2">
                <h3 class="font-bold text-center md:text-left text-[12px] md:text-xl">${watchlistData.title}</h3>
                ...
                <div class="flex justify-center md:justify-start items-center gap-2">
                    ...
                    <p class="text-[12px] md:text-[15px]">${watchlistData.vote_average.toFixed(1)}</p>
                    <img src="../assets/icons/star-icon.png" alt="star logo" class="w-[10px] md:w-[20px] ">
                </div>
                <p class="py-1 px-4 text-center md:text-left md:px-0 line-clamp-3 text-xs md:text-sm text-gray-300">${watchlistData.overview}</p>
                <div class="flex flex-col justify-center items-center md:items-start md:justify-start md:flex-row md:px-0 gap-2 py-2">
                    ...
                    <button data-id="${watchlistData.id}" class="btn-watchlist rounded-full text-xs md:text-sm border px-3 md:px-6 py-2 font-semibold">REMOVE FROM WATCHLISTS</button>
                </div>
            </div>
